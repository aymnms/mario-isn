...
- **Dette mineure identifiée, résolue en `J15-1` (2026-08-16)** : constantes magiques bypassant `MAX_MECHANTS` (8 sites avec `<= 19` en dur, pas 7 comme initialement compté ici) ; `extern` redondants — en réalité jusqu'à 8 fichiers redéclarant individuellement `statue[]`/`tableau_mechant[]` (plus large que "13 fichiers" ne le laissait supposer), maintenant centralisés dans `MARIO_mechant.h` ; globals `boolSB`/`taille` mortes (`deplacement_mechant.c`) ; `srand(time(NULL))` appelé à chaque frame par méchant alors que `rand()` n'est appelé nulle part dans tout le dépôt ; 8 des 16 headers du projet (`MARIO_*.h`) n'avaient aucun garde d'inclusion. Voir `PLAN.md` `J15-1`.
- **Non prioritaire, résolu en `J15-1`** : `Mix_LoadMUS`/`Mix_LoadWAV` (9 sites, `music.c`) désormais vérifiés (log via `Mix_GetError()` en cas d'échec, sans changer le flux de contrôle) ; `freeMusic()` libère maintenant les 9 ressources chargées, pas seulement `musiqueFond`. Non traité (toujours non prioritaire) : `create_texture()` vérifié en interne mais son retour rarement vérifié par l'appelant ; mélange français/anglais dans les noms — cohérent avec l'origine ISN du projet, pas une vraie dette.

## 11. Bilan de l'extraction domain `J14-2` à `J14-5` et décision sur `J14-6` (2026-08-16)

Les 4 phases obligatoires du plan d'extraction (§ `PLAN.md` "Plan d'extraction domain — J14+") sont terminées : `goM()`, `goB()`, la machine à états saut/chute, et le parsing de `niveau.lvl` ont chacune leur logique pure extraite dans `src/domain/`, testée en TDD, sans changement de comportement observable (vérifié à chaque étape par diff contre l'original — `goB()` a nécessité une fonction dédiée plutôt qu'une réutilisation de `domain_classify_tile()`, voir Journal `PLAN.md`). Couverture domaine passée de 53 à 90 assertions.

**`J14-6` (optionnel, `game_over()`/`victory()` dans `conditions.c`) évalué et non entrepris** — lecture complète des deux fonctions : la quasi-totalité du code est intrinsèquement couplée à SDL (chargement/affichage de texture, `SDL_Delay`, `SDL_WaitEvent`, boucle de rendu image par image de l'animation de victoire). Les seuls fragments réellement purs qui en émergeraient sont triviaux (ex. `niveau == 6 ? fin du jeu : niveau++`) — un gain de testabilité quasi nul pour un risque réel : casser le séquencement visuel de mort/victoire n'est vérifiable qu'en rejouant réellement (aucun test automatisé ne peut valider "l'animation a l'air correcte"), contrairement aux 4 phases précédentes où chaque fonction pure extraite avait une sémantique vérifiable par assertion. Conforme au principe KISS/YAGNI du skill APE (ne pas forcer une abstraction sans bénéfice de testabilité mesuré) : recommandation consignée ici plutôt qu'exécutée. À reconsidérer seulement si `conditions.c` devient un point de friction concret (bug récurrent, besoin réel de le tester).
