...
- [ ] Fix Node 22 pour `release.yml` (bug trouvé indépendamment sur `main`, cf. journal) — propagé sur les 4 branches, mais reste non vérifié par un run réel de `release.yml` (celui-ci ne se déclenche que sur des builds `main`) tant qu'aucune branche n'est mergée
